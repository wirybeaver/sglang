"""Shared tiny Gemma 4 fixtures for offline MLX tests."""
...
def tiny_gemma4(*, wrapped: bool = False):
    ...
    model = (
        gemma4.Model(
            gemma4.ModelArgs(text_config=asdict(args), vocab_size=args.vocab_size)
        )
        if wrapped
...
def native_cache_snapshot(cache):
    """Capture logical K/V in temporal order plus visible ring metadata."""
