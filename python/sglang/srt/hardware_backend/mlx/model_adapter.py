...
class Gemma4TargetAdapter:
    ...
    def input_embeddings(self, input_ids: mx.array) -> mx.array:
        embed_scale = float(getattr(self.backbone, "embed_scale", 1.0))
        return self.backbone.embed_tokens(input_ids) * embed_scale
    ...
    def make_seed(
        ...
        *,
        ...
        if hidden is None or hidden.ndim != 3:
            raise ValueError("target output has no batch-major hidden states")
        if hidden_row_index < 0 or hidden_row_index >= hidden.shape[1]:
            raise IndexError("target hidden row is outside the query width")
        ...
        return MlxTargetSeed(
            ...
            hidden_state=hidden[:, hidden_row_index : hidden_row_index + 1, :],
            token_embedding=self.input_embeddings(token_ids),
        )
